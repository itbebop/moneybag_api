...
const actionHandlers = {
  ...
  createAssetPallete: createAssetPallete,
  getAsset: getAsset,
  getAssetPallete: getAssetPallete,
};

assetRoutes.route("/:id").patch((req, res) => {
  const action = req.headers["action"]; // 요청 헤더에서 action 추출
  logger.info(`PATCH /asset/${req.params.id}, action: ${action}`);
...
assetRoutes.route("/").post((req, res) => {
  const action = req.headers["action"]; // 요청 헤더에서 action 추출
  logger.info(`POST /asset, action: ${action}`);
...
assetRoutes
  ...
  .get(getAssets)
  .post(createAsset)
  .post(createAssetPallete);

assetRoutes.route("/:id").get(getAsset).get(getAssetPallete);
